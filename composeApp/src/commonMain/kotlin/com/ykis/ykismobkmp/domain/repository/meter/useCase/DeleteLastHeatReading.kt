...
private const val tag = "UseCase.DeleteLastHeatReading"

/**
 * [DeleteLastHeatReading] — Единый КМР-стандарт интерактора удаления ошибочных показаний теплосчетчика.
    ...
    flow<Resource<GetSimpleResponse?>> { // Принудительно задаем тип контекста всего потока
      ...
      try {
        ...
        if (response.success == 1) {
          ...
          println("[$tag.$methodName]: Показання тепла $readingId успішно видалено з локальної СУБД")
