 ...
 * ИСПРАВЛЕНО: Вызовы конструктора Resource.Error переведены на оригинальный позиционный стандарт, убирая ошибку 'No parameter with name found'.
 */
class GetFamilyList(
    ...
    try {
      ...
      if (response.success == 1 && remoteFamily.isNotEmpty()) {
        ...
        if (cached.isEmpty()) {
          ...
          // ИСПРАВЛЕНО: Префикс 'message =' удален. Строка передана позиционно на первое место, как требует твой оригинальный Resource.kt
      ...
      if (fallback.isNotEmpty()) {
        ...
        // ИСПРАВЛЕНО: Префикс 'message =' удален, строка передана позиционно
