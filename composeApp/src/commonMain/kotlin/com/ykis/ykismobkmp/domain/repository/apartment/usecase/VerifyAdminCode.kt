...
class VerifyAdminCode(
  ...
  operator fun invoke(code: String, uid: String): Flow<Resource<GetSimpleResponse>> = flow {
    ...
    try {
      ...
      Log.d("YkisLog", "[$methodName]: [RESPONSE] Success: ${response.success}, Role: ${response.userRole}")

      // 2. ОБРАБОТКА РЕЗУЛЬТАТА
      if (response.success == 1) {
        Log.i("YkisLog", "[$methodName]: [SUCCESS] Доступ разрешен. Роль: ${response.userRole}")
